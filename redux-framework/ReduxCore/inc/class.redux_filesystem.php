<?php
    ...
    if ( ! class_exists( 'ThemegumRedux_Filesystem' ) ) {
        class ThemegumRedux_Filesystem {
            ...
            public function __construct() {
                $this->parent->admin_notices[] = array(
                    'type'    => 'error',
                    'msg'     => '<strong>' . esc_html__( 'File Permission Issues', 'lapindos' ) . '</strong><br/>' . sprintf( esc_html__( 'We were unable to modify required files. Please check your permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%s" target="_blank">outlined here</a>.', 'lapindos' ), 'https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants' ),
                    'id'      => 'redux-wp-login',
                    'dismiss' => false,
                );
            }
            ...
            public function ftp_form() {
                if ( isset( $this->parent->ftp_form ) && ! empty( $this->parent->ftp_form ) ) {
                    ...
                    echo '<strong>' . esc_html__( 'File Permission Issues', 'lapindos' ) . '</strong><br/>' . sprintf( esc_html__( 'We were unable to modify required files. Please ensure that <code>%1s</code> has the proper read-write permissions, or modify your wp-config.php file to contain your FTP login credentials as <a href="%2s" target="_blank">outlined here</a>.', 'lapindos' ), ThemegumRedux_Helpers::cleanFilePath( trailingslashit( WP_CONTENT_DIR ) ) . '/uploads/', 'https://codex.wordpress.org/Editing_wp-config.php#WordPress_Upgrade_Constants' );
        ...
        ThemegumRedux_Filesystem::get_instance();
    }
