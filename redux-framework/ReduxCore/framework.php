...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( class_exists( 'ThemegumReduxFrameworkInstances' ) ) {
        add_action( 'lapindos_redux/init', 'ThemegumReduxFrameworkInstances::get_instance' );
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'ThemegumReduxFramework' ) ) {
        ...
        class ThemegumReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
               ...
               // Disregard WP AJAX 'heartbeat'call.  Why waste resources?
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Options panel created using %1$s', 'lapindos' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'lapindos' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'lapindos' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'lapindos' );
                }
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    $this->filesystem = ThemegumRedux_Filesystem::get_instance( $this );
            ...
            private function set_default_args() {
                ...
                $this->args = array(
                    ...
                    'menu_icon'                 => '',
                    // menu icon
                    'menu_title'                => esc_html__( 'Theme Options', 'lapindos' ),
                    // menu title/text
                    'page_title'                => esc_html__( 'Theme Options', 'lapindos' ),
                    // option page title
            ...
            public function show( $opt_name, $default = '' ) {
                ...
                if ( ! is_array( $option ) && $option != '' ) {
                    ...
                    print $this->_get_default( $opt_name, $default );
                }
            }
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'lapindos' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'lapindos' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'lapindos' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'lapindos' );
                    }

                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'lapindos' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'lapindos' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'lapindos' ) . ": " . substr( $default_output, 0, - 2 );
                }
                ...
                if ( isset ( $field['title'] ) && isset ( $field['type'] ) && $field['type'] !== "info" && $field['type'] !== "section" ) {
                    ...
                    if ( isset ( $field['hint'] ) && ! '' == $field['hint'] ) {
                        ...
                        if ( 'true' == $metaVal || empty ( $metaVal ) ) {
                            ...
                            $hint = '<div class="redux-hint-qtip" style="float:' . $this->args['hints']['icon_position'] . '; font-size: ' . $size . '; color:' . $hint_color . '; cursor: ' . $pointer . ';" qtip-title="' .esc_attr( $titleParam ). '" qtip-content="' . esc_attr( $contentParam ). '">&nbsp;<i class="' . ( isset( $this->args['hints']['icon'] ) ? sanitize_html_class( $this->args['hints']['icon'] ) : '' ) . '"></i></div>';
                        }
                    }
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" ) ) {
                    json_encode( array(
                        'status' => esc_html__( 'Invalid security credential, please reload the page and try again.', 'lapindos' ),
                        'action' => 'reload'
                    ) );
                    die();
                }
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'lapindos' ) ) );
                    }
                }
            ...
            public function _field_input( $field, $v = null ) {
                ...
                if ( isset ( $field['type'] ) ) {
                    ...
                    if ( class_exists( $field_class ) ) {
                        ...
                        print '<fieldset id="' . $this->args['opt_name'] . '-' . $field['id'] . '" class="' . $hidden . 'redux-field-container redux-field redux-field-init redux-container-' . $field['type'] . ' ' . $class_string . '" data-id="' . $field['id'] . '" ' . $data_string . ' data-type="' . $field['type'] . '">';
                        ...
                        print ( isset ( $field['description'] ) && $field['type'] != "info" && $field['type'] !== "section" && ! empty ( $field['description'] ) ) ? '<div class="description field-desc">' . $field['description'] . '</div>' : '';
        ...
        /**
         * action 'lapindos_redux/init'
         *
         * @param null
         */

        do_action( 'lapindos_redux/init', ThemegumReduxFramework::init() );
