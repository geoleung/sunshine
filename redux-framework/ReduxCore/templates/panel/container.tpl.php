...
<div class="redux-container<?php echo esc_attr( $expanded ); ?>">
    ...
    <form method="post" action="<?php echo esc_url($action); ?>" data-nonce="<?php echo esc_attr($nonce); ?>" enctype="multipart/form-data"
        id="redux-form-wrapper">
        <input type="hidden" id="redux-compiler-hook"
            name="<?php echo wp_strip_all_tags($this->parent->args['opt_name']); ?>[compiler]"
            value=""/>
        <input type="hidden" id="currentSection"
            name="<?php echo wp_strip_all_tags($this->parent->args['opt_name']); ?>[redux-section]"
            value=""/>
        <?php if ( ! empty( $this->parent->no_panel ) ) : ?>
            <input type="hidden" name="<?php echo wp_strip_all_tags($this->parent->args['opt_name']); ?>[redux-no_panel]"
        ...
        ?>
        <input type="hidden" id="last_tab" name="<?php echo wp_strip_all_tags($this->parent->args['opt_name']); ?>[last_tab]"
